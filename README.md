...
plan allows enforcing reviewers), and the `ci / Type-check and test` check
must pass. Repository **admins bypass it entirely** (`actor_id: 5` is the
built-in Admin role) so release commits can still be pushed directly —
tighten that once the team grows. Plan gating: on the Free plan the rulesets
API **refuses private repos outright** (403); the script applies the plain
settings, says so, and exits cleanly — re-run it after the org moves to
Team+.

For the rules every Worker repo shares, prefer one org-level ruleset over
per-repo copies (available on Team+): edit the repo list in
[`org/worker-repos-ruleset.json`](org/worker-repos-ruleset.json), then

```bash
gh api -X POST orgs/Generality-Labs/rulesets --input org/worker-repos-ruleset.json
```

(org admin required; re-apply with `-X PUT orgs/…/rulesets/<id>` after edits).
